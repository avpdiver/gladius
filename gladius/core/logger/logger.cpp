...
const char
	*ROW = "<tr style='background-color:%s'><td>%i</td><td>%s</td><td>%s</td><td>%s</td><td>%i</td><td>%s</td></tr>\n";
const size_t ROW_LEN = std::char_traits<char>::length(ROW) + 7; // 7 for web color format #rrggbb

const std::array<const char *, ((size_t) (e_log_level::debug) + 1)>
	LEVELS {{"FATAL", "ERROR", "WARN", "INFO", "DEBUG"}};
const std::array<const char *, ((size_t) (e_log_level::debug) + 1)>
	LEVELS_COLORS {{"#ff0000", "#ff4000", "#ffff00", "#009800", "#0000ff"}};
...
void log(e_log_level level, const char *type, const char *filename, int line, const char *format, ...) {
	...
	while (true) {
		...
		if (w > 0 && w < (int) g_local_buffer.size()) {
			...
			buffer->resize(len);
			w = std::snprintf(&(buffer->at(0)),
							  buffer->length(),
							  ROW,
							  LEVELS_COLORS[(size_t) level],
							  time.count(),
							  type,
							  LEVELS[(size_t) level],
							  filename,
...
void shutdown() {
	log(e_log_level::info, "LOGGER", __FILENAME__, __LINE__, "Shutdown logger system");
	g_log_queue.push(nullptr);
