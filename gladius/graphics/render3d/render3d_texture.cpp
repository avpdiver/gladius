...
bool create_staging_texture(VkImageCreateInfo &image_create_info, s_texture &texture, gli::texture2d &tex2d) {
    ...
    for (uint32_t level = 0; level < texture.mip_levels; level++) {
        ...
        VERIFY_LOG(mem_alloc_info.memoryTypeIndex >= 0, LOG_TYPE, "There is no required memory type", "");
...
bool create_mapped_texture(VkImageCreateInfo &image_create_info, s_texture &texture, gli::texture2d &tex2d) {
    ...
    mem_alloc_info.memoryTypeIndex =
        utils::get_memory_type(mem_reqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
...
bool load_texture(const char *filename, uint32_t image_format) {
    ...
    if (use_staging) {
        VERIFY_LOG(create_staging_texture(image_create_info, texture, tex2d), LOG_TYPE,
                   "Failed create staged texture", "");
    } else {
        VERIFY_LOG(create_mapped_texture(image_create_info, texture, tex2d), LOG_TYPE,
